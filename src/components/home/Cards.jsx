...
const Cards = () => {
  ...
  return (
    <div className='grid grid-cols-4 gap-[3.083vw]'>
      { movie !== null && (
        movie.map((item) => (
          <Link href={`/movie/${item.id}`}>
            <div key={item.id} className='flex flex-col items-center gap-[0.903vw]'>
                <Image src={item.image} alt='alt' width={250} height={375} className='w-[17.35vw] h-[26.0417vw] hover:opacity-[70%] cursor-pointer'/>
